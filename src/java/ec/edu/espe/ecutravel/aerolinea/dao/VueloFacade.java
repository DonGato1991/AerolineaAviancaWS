...
public class VueloFacade extends AbstractFacade<Vuelo> {
    ...
    @PersistenceContext(unitName = "AerolineaAviancaWSPU")
...
//                    .getResultList();
            ...
            vuelos = em.createQuery("SELECT v from Vuelo v where v.ciudadOrigen=:origen and v.ciudadDestino=:destino and (FUNC('TO_CHAR', v.fechaInicio) like :inicio or FUNC('TO_CHAR', v.fechaFin) like :fin )").setParameter("origen", origen).setParameter("inicio", '%' + inicio + '%')
